...
async function loadDossier() {
    ...
    const res = await api(`/rental_dossiers/${dossierId}`, "GET");
    ...
    infoBox.innerHTML = `
        ...
        <p><strong>Véhicule :</strong> ${dossier.vehicle?.brand || ""} ${dossier.vehicle?.model || ""}</p>

        <p>
            <strong>Status :</strong>
            <span class="badge bg-warning">${dossier.status}</span>
        </p>

        <p><strong>Message :</strong> ${dossier.message || "-"}</p>

        <p><strong>Date :</strong> ${new Date(dossier.created_at).toLocaleDateString()}</p>
    `;
    ...
    tableBody.innerHTML = dossier.documents.map(doc => `
        <tr>
            ...
            <td>
                <a
                    href="${FILE_BASE}/api/v1/documents/file/${doc.filename}"
                    target="_blank"
